...
if [ "$1" = 'catalina.sh' ]; then
	...
	sed -ri '/^jdbc[.](username|password|database|host|port)=/d' "$CATALINA_HOME"/webapps/catalogue/WEB-INF/config-db/jdbc.properties
	echo "jdbc.username=$POSTGRES_DB_USERNAME" >> "$CATALINA_HOME"/webapps/catalogue/WEB-INF/config-db/jdbc.properties
	echo "jdbc.password=$POSTGRES_DB_PASSWORD" >> "$CATALINA_HOME"/webapps/catalogue/WEB-INF/config-db/jdbc.properties
	echo "jdbc.database=$db_gn" >> "$CATALINA_HOME"/webapps/catalogue/WEB-INF/config-db/jdbc.properties
	echo "jdbc.host=$db_host" >> "$CATALINA_HOME"/webapps/catalogue/WEB-INF/config-db/jdbc.properties
	echo "jdbc.port=$db_port" >> "$CATALINA_HOME"/webapps/catalogue/WEB-INF/config-db/jdbc.properties

	#Fixing an hardcoded port on the connection string (bug fixed on development branch)
	sed -i -e 's#5432#${jdbc.port}#g' $CATALINA_HOME/webapps/catalogue/WEB-INF/config-db/postgres.xml
fi
